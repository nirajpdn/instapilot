...
export async function processCommentTarget(payload: CommentTargetJobPayload) {
  await logTargetEvent(
    payload.targetId,
    "INFO",
  ...
  const claim = await prisma.commentJobTarget.updateMany({
    where: { id: payload.targetId, status: CommentTargetStatus.QUEUED },
    data: { status: CommentTargetStatus.RUNNING, startedAt: new Date() },
  });
  if (claim.count === 0) {
    return { skipped: true as const, reason: "Target already claimed" };
  }
  ...
  try {
    ...
    if (
      ...
      await refreshJobAggregate(payload.jobId);
      return { skipped: true as const, reason: "ACCOUNT_NOT_READY" };
    }
    ...
    if (controlBeforeWork.action === "cancel") {
      ...
      await refreshJobAggregate(payload.jobId);
      return { skipped: true as const, reason: "JOB_CANCELED" };
    }
...
export async function processQueuedTargets(limit = 1) {
  const targets = await prisma.commentJobTarget.findMany({
    where: {
    ...
    select: {
      ...
      jobId: true,
      accountId: true,
      job: { select: { normalizedPostUrl: true } },
    },
  });
  ...
  for (const t of targets) {
    try {
      const result = await processCommentTarget({
        targetId: t.id,
        jobId: t.jobId,
